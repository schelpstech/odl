   ...
   <link rel="shortcut icon" type="image/x-icon" href="https://rabbischools.com.ng/press/wp-content/uploads/2020/04/icon.jpg">
    ...
    <script>
      function aware() {
        ...
        confirm("You are deleting a class allocation record?");
      ...
      $term = $row['term'];
           ...
           $query->execute();
            ...
            $row = mysqli_fetch_array($result);
              ...
              $sql = " SELECT * from lhpclassalloc where classid = '$classid' and term = '$term'";
              $result = mysqli_query($con,$sql);
              $row = mysqli_fetch_array($result);
              ...
              $tutor = $row['staffname'];
                ...
                $tutor = "Unassigned";
                $assign = '<a href="allocateclass.php?classid='.$classid.'" type="button" class="btn btn-success"><strong>Assign Class Teacher</strong></a>';
              }
        ...
        <td><strong><?php echo $population ?></strong></td>
				<td><strong><?php echo $tutor ?></strong></td>
