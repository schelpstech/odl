...
if(!empty($_GET['termid'])) {
	...
	$query ="SELECT DISTINCT(classid) FROM lhpresultrecord WHERE term = '$termid'";
...
if(!empty($_GET['cld'])) {
        $classid = $_GET["cld"];           
	$query ="SELECT DISTINCT(lid) FROM lhpresultrecord WHERE term = '$_SESSION[term]' and classid = '$classid'";
	$results = $db_handle->runQuery($query);
?>
