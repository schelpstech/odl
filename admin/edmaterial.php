...
if (isset($_POST['dellsn']) && $_POST['dellsn'] == 'Delete Learning Material Details')
{
    $ln = mysqli_real_escape_string($con,$_POST['lsnid']);
    
     $sql= "DELETE FROM classact  WHERE id = '$ln'";
    
    	if(mysqli_query($con, $sql)){	
		
		$mes = 'Status : Successfully DELETED learning material detail.';
		}

      else 
      {
        $mes ='Status : Unable to DELETE learning material detail.';
      }
    }
	$_SESSION['mes'] = $mes;
	header("Location: mglesson.php");   
?>